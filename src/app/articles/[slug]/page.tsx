    ...
    return (
        <div className="mx-auto max-w-3xl">
            <nav aria-label="パンくずリスト" className="mb-8 text-sm text-amber-950/55">
                ...
                <span>{article.categorySubgroup ?? article.category}</span>
                <span className="mx-2">&gt;</span>
                <span aria-current="page">{article.title}</span>
            </nav>

            <header>
                <p className="text-sm font-semibold text-amber-700">{article.categorySubgroup ? `${article.categoryGroup} / ${article.categorySubgroup}` : article.categoryGroup ? `${article.categoryGroup} / ${article.category}` : article.category}</p>
