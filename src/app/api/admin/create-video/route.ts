...
export async function POST(req: NextRequest) {
    ...
    const { title, slug, url } = body as { title: string; slug: string; url: string };
    if (!title || !slug || !url) return NextResponse.json({ message: 'Missing fields' }, { status: 400 });
    ...
    const createRes = await fetch(`https://api.github.com/repos/${OWNER}/${REPO}/contents/${encodeURIComponent(path)}`, {
        ...
        body: JSON.stringify({ message: `Add video ${slug}`, content: Buffer.from(md).toString('base64'), branch: BRANCH })
    });
